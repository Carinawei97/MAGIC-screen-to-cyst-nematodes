...
class CondInst(nn.Module):
    ...
    def forward(self, batched_inputs):
        original_images = [x["image"].to(self.device) for x in batched_inputs] #image放入device

        # normalize images
        images_norm = [self.normalizer(x) for x in original_images]
        images_norm = ImageList.from_tensors(images_norm, self.backbone.size_divisibility)  # torch.size([2,3,768,1248])

        features = self.backbone(images_norm.tensor) #forward build_fcos_resnet_fpn_backbone len=1

        if "instances" in batched_inputs[0]:
            gt_instances = [x["instances"].to(self.device) for x in batched_inputs]  #len(gt_instance) = batch_size, 一共有gt_instance[0:batch_size]
        ...
        if self.training:
            mask_losses = self._forward_mask_heads_train(proposals, mask_feats, gt_instances)   #调用_forward_mask_heads_train
            ...
            pred_instances_w_masks = self._forward_mask_heads_test(proposals, mask_feats)   #调用_forward_mask_heads_test
            ...
            for im_id, (input_per_image, image_size) in enumerate(zip(batched_inputs, images_norm.image_sizes)):
                ...
                instances_per_im = self.postprocess(   #调用postprocess
    ...
    def _forward_mask_heads_train(self, proposals, mask_feats, gt_instances):
        ...
        pred_instances = proposals["instances"]   # len 1
        ...
        if self.max_proposals != -1:
            ...
            pred_instances = Instances.cat(kept_instances)  # [160, 169]
