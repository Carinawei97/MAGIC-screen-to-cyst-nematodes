...
def check_multi_preview_window():
    ### this fucntion checks if there is already one of the preview windows open from libcamera
    #check which programmes are running in terminal : ps aux | grep programname
    ## have to check and try what the process is called
    a = True
    while a == True:
        try:
            out = check_output(['pgrep', 'libcamera-still'])
            time.sleep(3)
            subprocess.Popen(["kill -9 $(pgrep -f 'libcamera-still')"], shell=True, stdout=subprocess.PIPE)
            subprocess.Popen(["kill -9 $(pgrep -f 'libcamera-still')"], shell=True, stdout=subprocess.PIPE)
            subprocess.Popen(["kill -9 $(pgrep -f 'libcamera-still')"], shell=True, stdout=subprocess.PIPE)
        except:
            print('')
...
def take_image():
	...
	global check
	global state
	check_multi_preview_window()
	state_list_check = []
	state = 'wait'
	relay_on(lights)
	check = 1
	time.sleep(0.05)
	t = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE)
	readt = t.stdout.read
	error_mess = r"*.ERROR.*"
	if error_mess in str(readt):
		print('failed')
	else:
		pass
	image_check()
...
def image_check():
	...
	time_string = time_now.strftime("%Y-%m-%d_%H_%M_%S.%f")
	while check == 1:
